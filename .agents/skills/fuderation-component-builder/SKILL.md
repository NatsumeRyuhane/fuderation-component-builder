...
| [SITE_NATIVE_KIT.md](SITE_NATIVE_KIT.md) | **Optional** kit for components that should look like native `chat.fuderation.com` UI — site token contract, the wrapper-alias rule that works in both render modes, DSL vs iframe tiers, and the pattern library in `assets/site-native/`. Use it for chrome (panels, readouts, choice lists); skip it for set pieces with their own aesthetic. Its §2 wrapper-alias rule is worth reading either way. |
